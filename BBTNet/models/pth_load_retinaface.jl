...
function get_pth_resnet_params(data; dtype=Array{Float32})
    ...
    push!(conv_w, convert(dtype, permutedims(data["module.body.conv1.weight"], (4, 3, 2, 1))))
...
function set_pth_fpn_data(model::RetinaFace, data; dtype=Array{Float32})
    ...
    model.fpn.o5.conv.w = Param(convert(dtype, permutedims(data["module.fpn.output3.0.weight"], (4, 3, 2, 1))))
    ...
    model.fpn.o4.conv.w = Param(convert(dtype, permutedims(data["module.fpn.output2.0.weight"], (4, 3, 2, 1))))
    ...
    model.fpn.o3.conv.w = Param(convert(dtype, permutedims(data["module.fpn.output1.0.weight"], (4, 3, 2, 1))))
    ...
    # Merge connections
    model.fpn.merge4.conv.w = Param(convert(dtype, permutedims(data["module.fpn.merge2.0.weight"], (4, 3, 2, 1))))
    ...
    model.fpn.merge3.conv.w = Param(convert(dtype, permutedims(data["module.fpn.merge1.0.weight"], (4, 3, 2, 1))))
...
function set_pth_head_data(model::HeadGetter, start_txt, data; dtype=Array{Float32})
    model.layers[2].w = Param(convert(dtype, permutedims(data[start_txt * ".0.conv1x1.weight"], (4, 3, 2, 1))))
    model.layers[2].b = Param(convert(dtype, 
        reshape(data[start_txt * ".0.conv1x1.bias"], (1, 1, size(data[start_txt * ".0.conv1x1.bias"], 1), 1))))
    
    model.layers[3].w = Param(convert(dtype, permutedims(data[start_txt * ".1.conv1x1.weight"], (4, 3, 2, 1))))
    model.layers[3].b = Param(convert(dtype, 
        reshape(data[start_txt * ".1.conv1x1.bias"], (1, 1, size(data[start_txt * ".1.conv1x1.bias"], 1), 1))))
    
    model.layers[4].w = Param(convert(dtype, permutedims(data[start_txt * ".2.conv1x1.weight"], (4, 3, 2, 1))))
...
function set_pth_ssh_data(model::SSH, start_txt, data; dtype=Array{Float32})
    model.conv128.conv.w = Param(convert(dtype, permutedims(data[start_txt * ".conv3X3.0.weight"], (4, 3, 2, 1))))
    ...
    model.conv64_1_1.conv.w = Param(convert(dtype, permutedims(data[start_txt * ".conv5X5_1.0.weight"], (4, 3, 2, 1))))
    ...
    model.conv64_1_2.conv.w = Param(convert(dtype, permutedims(data[start_txt * ".conv5X5_2.0.weight"], (4, 3, 2, 1))))
    ...
    model.conv64_2_1.conv.w = Param(convert(dtype, permutedims(data[start_txt * ".conv7X7_2.0.weight"], (4, 3, 2, 1))))
    ...
    model.conv64_2_2.conv.w = Param(convert(dtype, permutedims(data[start_txt * ".conv7x7_3.0.weight"], (4, 3, 2, 1))))
...
function get_pth_block_data(data, stagename; dtype=Array{Float32})
    ...
    for k in sort(kvals)
        # print(k, " / ", stagename, " --> ",  occursin(k, "conv"), "\n")
        # downsample parameters
        if typeof(data[k]) == Int64
            continue
        end
        val = convert(dtype, data[k])
        if startswith(k, "module.body." * stagename) && endswith(k, "downsample.0.weight")
            conv_ds = permutedims(val, (4, 3, 2, 1))
        elseif startswith(k, "module.body." * stagename) && endswith(k, "downsample.1.bias")
            ds_bn_b = vec(val)
        ...
        elseif startswith(k, "module.body." * stagename) && endswith(k, "downsample.1.running_var")
            push!(ds_bn_moms, reshape(val, (1, 1, size(val, 1), 1)))     
        # residual parameters
        elseif startswith(k, "module.body." * stagename) && occursin("conv", k) && endswith(k, "weight")
            push!(conv_w, permutedims(val, (4, 3, 2, 1)))
